...
use super::{
    HandleShakeError, PosReader, SignatureHash, EXT_TYPE_ALPN, EXT_TYPE_NEXT_PROTO_NEG,
    EXT_TYPE_RENEGOTIATION_INFO, EXT_TYPE_SCT, EXT_TYPE_SESSION_TICKET,
    EXT_TYPE_SIGNATURE_ALGORITHMS, EXT_TYPE_STATUS_REQUEST, EXT_TYPE_SUPPORTED_CURVES,
    EXT_TYPE_SUPPORTED_POINTS, EXT_TYPE_SVR_NAME, STATUS_TYPE_OCSP,
};
...
impl<'a, 'b> ExtParser<'a, 'b> {
    ...
    fn parse_alpn(self) -> Result<Vec<String>, HandleShakeError> {
        if self.extension_len < 2 {
            return Err(HandleShakeError::ParserError);
        }
        let len = self.pos_reader.get_u16();
        ...
        while pos < len {
            ...
            alpn.push(unsafe { String::from_utf8_unchecked(b.to_vec()) });
        }
        Ok(alpn)
    }
}
...
impl<'a, 'b> ClientHelloExtensionsParser<'a, 'b> {
    ...
    fn parser(&mut self) -> Result<ClientHelloExtension, HandleShakeError> {
        let mut client_hello_ext = ClientHelloExtension::default();
        loop {
            ...
            match ext_type {
                EXT_TYPE_SVR_NAME => {
                    let parser = ExtParser::new(&mut self.pos_reader, ext_len);
                    client_hello_ext.server_name = parser.parse_server_name()?;
                }
                ...
                EXT_TYPE_SIGNATURE_ALGORITHMS => {
                    client_hello_ext.signature_algorithms =
                        parse_extension!(parse_signature_algorithms);
                }
                EXT_TYPE_ALPN => {
                    client_hello_ext.alpn = parse_extension!(parse_alpn);
                }
                EXT_TYPE_RENEGOTIATION_INFO => {
                    client_hello_ext.secure_renegotiation =
                        parse_extension!(parse_renegotiation_info);
                }
                EXT_TYPE_SCT => {
                    if ext_len != 0 {
                        return Err(HandleShakeError::ParserError);
                ...
                _ => self.pos_reader.advance(ext_len as _),
            }
        }
        Ok(client_hello_ext)
    }
...
mod test {
    ...
    fn test_client_hello_parse() {
        const packet: &[u8] = &[
            ,
        ];
